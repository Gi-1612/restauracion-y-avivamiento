...
// por endpoint, esta única función atiende a todos según la URL pedida
// (ej: /api/admin/categorias/5 -> action = ["categorias", "5"]).
...
export default async function handler(req, res) {
  const [recurso, id] = Array.isArray(req.query.action) ? req.query.action : [req.query.action];
